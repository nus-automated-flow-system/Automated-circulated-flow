...
def delete():
    ...
    txt_output_cmd.pop(fr_cmdInt.curselection()[0])
    txt_output_interface.pop(fr_cmdInt.curselection()[0])
    fr_cmdInt.delete(fr_cmdInt.curselection()[0])
...
def instru_init(instru, num, COM):
    ...
    fr_cmdInit.insert(tk.END, "Initialize %s %s at %s" %(instru, num, COM))
...
def asiapump_pumping(num, chan, timeEntry, rateEntry):  
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Asiapump %d channel %d pumps at %d ul/min" %(time, num, (chan+1), rate))
        timeline.append(time)

        #write the interface to file

        txt_output_interface.append("After %.1f min, Asiapump %d channel %d pumps at %d ul/min"%(time, num, (chan+1), rate))
    
        #write the command to file

        txt_output_cmd.append("\tasiapump_%d.asiapump_pumping(%d, %.1f, %d," %(num, chan, time, rate))
...
def asiapump_filling(num, chan, timeEntry, rateEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Asiapump %d channel %d fills at %d ul/min" %(time, num, (chan+1), rate))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, Asiapump %d channel %d fills at %d ul/min"%(time, num, (chan+1), rate))

        txt_output_cmd.append("\tasiapump_%d.asiapump_filling(%d, %.1f, %d,"%(num, chan, time, rate))
...
def asiapump_emptying(num, chan, timeEntry, rateEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Asiapump %d channel %d empties at %d ul/min" %(time, num, (chan+1), rate))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, Asiapump %d channel %d empties at %d ul/min"%(time, num, (chan+1), rate))

        txt_output_cmd.append("\tasiapump_%d.asiapump_emptying(%d, %.1f, %d,"%(num, chan, time, rate))
...
def asiapump_stoping(num, chan, timeEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Asiapump %d channel %d stops" %(time, num, (chan+1)))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, Asiapump %d channel %d stops"%(time, num, (chan+1)))

        txt_output_cmd.append("\tasiapump_%d.asiapump_stoping(%d, %.1f,"%(num, chan, time))
...
def sf10_pumping(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface. 
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, SF10 %d valve %s pumps at %.1fml/min" %(time, num, chan, rate))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, SF10 %d valve %s pumps at %.1fml/min"%(time, num, chan, rate))

        txt_output_cmd.append("\tsf10_%d.sf10_pumping('%s', %.1f, %.1f,"%(num, chan, time, rate))
...
def sf10_gaspumping(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface. 
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, SF10 %d valve %s gaspumps at %.1fml/min" %(time, num, chan, rate))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, SF10 %d valve %s gaspumps at %.1fml/min"%(time, num, chan, rate))

        txt_output_cmd.append("\tsf10_%d.sf10_gaspumping('%s', %.1f, %.1f,"%(num, chan, time, rate))
...
def watson_pumping(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface. 
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Watson Marlow %d direction %s pumps at %.1fRPM" %(time, num, chan, rate))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, Watson Marlow %d direction %s pumps at %.1f RPM"%(time, num, chan, rate))

        txt_output_cmd.append("\twatson_%d.watson_pumping('%s', %.1f, %.1f,"%(num, chan, time, rate))
...
def watson_dosing(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface. 
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Watson Marlow %d direction %s dose %.1f tacho" %(time, num, chan, rate))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, Watson Marlow %d direction %s dose %.1f tacho"%(time, num, chan, rate))

        txt_output_cmd.append("\twatson_%d.watson_dosing('%s', %.1f, %.1f,"%(num, chan, time, rate))
...
def watson_stoping(num, chan, timeEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface. 
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.1f min, Watson Marlow %d direction %s stops" %(time, num, chan))
        timeline.append(time)

        txt_output_interface.append("After %.1f min, Watson Marlow %d direction %s stops"%(time, num, chan))

        txt_output_cmd.append("\twatson_%d.watson_stoping('%s', %.1f,"%(num, chan, time))
...
fr_cmdInit_scroll = tk.Frame(tab_output, relief=tk.FLAT, bd=2)
fr_cmdInit_scroll.pack()

scrollbar_init = tk.Scrollbar(fr_cmdInit_scroll)
scrollbar_init.pack(side=tk.RIGHT, fill=tk.Y)
fr_cmdInit = tk.Listbox(fr_cmdInit_scroll, width=85, height=30, selectmode= tk.SINGLE, yscrollcommand = scrollbar_init.set)
fr_cmdInit.pack(side =tk.LEFT, fill=tk.BOTH)
scrollbar_init.configure(command = fr_cmdInit.yview) 

tab_output.add(fr_cmdInit_scroll, text = "Init")
...
tab_output.add(fr_cmdInt_scroll, text = "Command")
...
btn_open.grid(row=0, column=1, padx=15, pady=3)

btn_clear = tk.Button(fr_cmdBtn, width=8,text = "Clear", command = txtclear)
btn_clear.grid(row=0, column=2, padx=15, pady=3)

btn_open = tk.Button(fr_cmdBtn, width=8, text = "Insert", command = insert)
btn_open.grid(row=0, column=3, padx=15, pady=3)

btn_save = tk.Button(fr_cmdBtn, width=8, text = "Delete", command = delete)
btn_save.grid(row=0, column=4, padx=15, pady=3)

btn_fig = tk.Button(fr_cmdBtn, width=8, text = "Figure", command = gen_fig)
btn_fig.grid(row=0, column=5, padx=15, pady=3)

btn_save = tk.Button(fr_cmdBtn, width=8, text = "Save", command = savefile)
btn_save.grid(row=0, column=6, padx=15, pady=3)
...
tk.Label(fr_watson_fn, text = "Anti-clockwise", width=12, height=1).grid(row=0, column=1)
tk.Label(fr_watson_fn, text = "Clockwise", width=10, height=1).grid(row=0, column=4)
